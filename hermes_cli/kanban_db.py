...
def complete_task(
    ...
    workstation_data = (metadata or {}).get("workstation")
    owned = conn.execute("SELECT created_by FROM tasks WHERE id = ?", (task_id,)).fetchone()
    hybrid_owned = conn.execute(
        "SELECT 1 FROM hybrid_card_delegations WHERE agent_task_id = ? LIMIT 1", (task_id,)
    ).fetchone()
    if not admit_task_completion(conn, task_id, workstation_data, owned, bool(hybrid_owned)):
        return False
    with write_txn(conn):
        ...
        if not _parents_satisfied(conn, task_id):
            return False
        # Revalidate the same ownership/admission facts inside the canonical write
        # transaction so a concurrent owner/run change cannot slip between prepare
        # and the terminal DONE update.
        owned = conn.execute("SELECT created_by FROM tasks WHERE id = ?", (task_id,)).fetchone()
        hybrid_owned = conn.execute(
            "SELECT 1 FROM hybrid_card_delegations WHERE agent_task_id = ? LIMIT 1", (task_id,)
        ).fetchone()
