...
def register_uri_scheme_read_handler(scheme: str, handler: Any) -> None:
    """Register a trusted read handler for a non-filesystem URI scheme."""
    _URI_SCHEME_READ_HANDLERS[scheme] = handler


def register_file_read_projection(handler: Any) -> None:
    """Register a final result projection without replacing file-read authority."""
...
def read_file_tool(path: str, offset: int = 1, limit: int = DEFAULT_READ_LIMIT, task_id: str = "default") -> str:
    ...
    try:
        if "://" in path:
            handler = _URI_SCHEME_READ_HANDLERS.get(path.split("://", 1)[0])
            if handler is not None:
                return handler(
                    path, offset=offset, limit=limit, task_id=task_id,
                    max_content_bytes=_get_max_read_chars(),
                )
