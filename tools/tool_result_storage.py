...
def _build_persisted_message(preview: str, has_more: bool, original_size: int,
                             file_path: str, *, original_bytes: int | None = None,
                             content_hash: str | None = None, result_ref: str | None = None,
                             cache_status: str = "miss") -> str:
    ...
    if result_ref and content_hash:
        metadata = (
            "Reference-first result metadata:\n"
            f"status: {'unchanged' if cache_status == 'hit' else 'ok'}\n"
            f"result_ref: {result_ref}\nartifact_ref: {file_path}\n"
            f"content_hash: {content_hash}\n"
            f"bytes: {original_bytes if original_bytes is not None else original_size}\n"
...
def maybe_persist_tool_result(content: str, tool_name: str, tool_use_id: str, env=None,
                              ...
                              result_scope: str | None = None) -> str:
...
def enforce_turn_budget(tool_messages: list[dict], env=None,
                        config: BudgetConfig = DEFAULT_BUDGET, *,
                        result_scope: str | None = None) -> list[dict]:
