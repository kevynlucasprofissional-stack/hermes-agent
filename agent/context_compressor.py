...
_TRUSTED_OPERATIONAL_REF_SOURCES = frozenset({
    "SessionOwner", "KanbanRun", "BrowserTask", "WorkerRegistry",
    "EvidenceState", "Approval", "ExecutionJournal",
})
_OPERATIONAL_REF_ID_FIELDS = (
    "id", "task_id", "session_id", "worker_id", "browser_task_id",
    "result_ref", "artifact_ref", "evidence_ref", "recovery_id",
)
...
def _validated_operational_refs(message: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for raw in raw_refs:
        if not isinstance(raw, dict) or raw.get("trusted") is not True:
            continue
        source, version, kind = raw.get("source"), raw.get("version"), raw.get("kind")
        ...
        if not isinstance(kind, str) or not kind.strip() or len(kind) > 80:
            continue
        identity = {
            field: value for field in _OPERATIONAL_REF_ID_FIELDS
            if isinstance((value := raw.get(field)), str)
            and 1 <= len(value) <= 256 and "\n" not in value and "\r" not in value
        }
        if not identity:
            continue
        ...
        validated.append({"kind": kind.strip(), "source": source, "version": version, **identity})
    return validated
...
def _build_operational_reference_envelope(turns: List[Dict[str, Any]]) -> str:
    ...
    refs = [ref for message in turns if isinstance(message, dict)
            for ref in _validated_operational_refs(message)]
    values: list[str] = []
    seen: set[str] = set()
    for ref in refs:
