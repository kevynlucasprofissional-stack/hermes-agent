...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    usage = getattr(agent, "_current_provider_usage", None)
    if usage is not None:
        msg["_hermes_token_count"] = usage.get("total_tokens")
...
def _should_skip_fallback_candidate(agent, fb: dict, fb_key: tuple, fb_provider: str, fb_model: str, unavailable: set) -> bool:
    ...
    if turn_constraints is not None:
        try:
            turn_constraints.require_provider(fb_provider, fb.get("base_url") or "")
        except Exception:
            logger.info("Fallback skip: %s/%s forbidden by turn constraints", fb_provider, fb_model)
            return True
