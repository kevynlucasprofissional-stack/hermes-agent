...
type LineageRow = Pick<SessionInfo, '_lineage_root_id' | 'id' | 'parent_session_id'>
...
/** Conversation ids accepted at ownership boundaries. Parent continuity is
 * directional: the selected/live child may reuse ownership recorded on its
 * direct parent, but selecting a parent never aliases every branch child. */
export function conversationAliases(storedId: string, sessions: readonly LineageRow[]): string[] {
  ...
  for (const session of sessions) {
    if (aliases.includes(session.id) || (session._lineage_root_id && aliases.includes(session._lineage_root_id))) {
